...
import { InHouseGuestSearchQueryDto } from './in-house-guest-search-query.dto';
import { RecordPosOrderPaymentDto } from './record-pos-order-payment.dto';
import { RestaurantDashboardQueryDto } from './restaurant-dashboard-query.dto';
import { RestaurantSalesSummaryQueryDto } from './restaurant-sales-summary-query.dto';
...
describe('Restaurant DTO validation', () => {
  ...
  it('rejects oversized POS close notes', async () => {
    const errors = await validationErrors(ClosePosOrderDto, {
      notes: 'N'.repeat(501),
    });
